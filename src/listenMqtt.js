			...
			if (i == (v.delta.attachments || []).length) {
				...
				if (fmtMsg) {
					...
					if (global.client && global.client.handleReply) {
						const handleReply = global.client.handleReply.find(r => r.messageID === fmtMsg.messageReplyTo);
						if (handleReply) {
							require("./handleMessageReply")({ api, event: fmtMsg, handleReply });
						}
					}
				}
	...
	if (v.delta.class == "ClientPayload") {
		...
		if (clientPayload && clientPayload.deltas) {
			for (const i in clientPayload.deltas) {
				...
				// Emoji reactions are now handled directly in the event handler
				//const handleReaction = require('./handleEmojiReaction')(defaultFuncs, api, ctx);
				//handleReaction(globalCallback)(delta);
				if (delta.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
					...
					// Handle via reaction handler first
					const reactionHandler = require('../../script/reactionHandler');
					reactionHandler.handle({ api, event: reactionEvent });
					
					// Then emit the event globally
					(function () {
